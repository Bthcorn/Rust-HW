...
pub fn cal_average_area(layers: &[Layer]) -> Vec<(String, f64, f64, f64)> {
    ...
    for layer in layers {
        ...
        let mut count = 0;
        let mut iter = layer.objects.iter();
        if let Some(circle) = iter.next() {
            let area = PI*(circle.r*circle.r);
            max = area;
            min = area;
            while let Some(circle) = iter.next() {
                let area = PI*((circle.r*circle.r));
                total_area += area;
                count += 1;
                if area >= max {
        ...
        result.push((layer.name.clone(), average_area, min, max));
    }
    result
}
...
pub fn gen_html(list: &[(String, f64, f64, f64)]) -> String {
    let mut html = String::from("<style>\ntable, td, th {\n\tborder: 1px solid #000000;\n\tborder-collapse: collapse;\n}\n</style>");
    html.push_str("\n<table>");
    html.push_str("\n\t<tr>\n\t<th>Layer</th>\n\t<th>Average Area</th>\n\t<th>Minimum</th>\n\t<th>Maximum</th>\n\t</tr>");

    for (name, area, min, max) in list {
        html.push_str("\n\t<tr>");
        html.push_str(&format!("\n\t\t<td>{}</td>\n\t\t<td>{}</td>\n\t\t<td>{}</td>\n\t\t<td>{}</td>", name, area, min, max));
        html.push_str("\n\t</tr>");
    }
    html.push_str("\n</table>");
    html
