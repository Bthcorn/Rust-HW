...
fn test_ascending_sort() {
    let mut array = vec![];
    let sorted = sort_value2::ascending_sort(&mut array);
...
fn test_descending_sort() {
    let mut array = vec![];
    let sorted = sort_value2::descending_sort(&mut array);
    assert_eq!(sorted, vec![]);
...
#[test]
fn test_sort_value() {
    let mut cmd: Command = Command::cargo_bin("sort_value2").unwrap();
    cmd.arg("4")
        .arg("7")
        .arg("8")
        .arg("9")
